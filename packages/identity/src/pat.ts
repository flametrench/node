...
/**
 * security-audit-v0.3.md H6 — pre-rejection cap on the secret-segment
 * length to defend against Argon2 DoS amplification. An attacker who
 * knows or guesses a real `pat_<id>_` prefix can otherwise submit
 * multi-MB secrets per request to force expensive Argon2id hashing.
 *
 * Real PAT secrets generated by `createPat` are 43 chars (32 random
 * bytes encoded as base64url no padding). 256 leaves a generous
 * margin for any future format extension while bounding the attack
 * surface; secrets longer than this are rejected before Argon2id
 * dispatches, conflated as `InvalidPatTokenError` like every other
 * structurally invalid bearer.
 */
